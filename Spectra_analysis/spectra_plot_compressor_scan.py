...
N_sample= len(os.listdir(pwd+'/'+os.listdir(pwd)[0])) #Total No.of spectra taken
im_i=list(range(1,N_sample+1))

N_i= len(os.listdir(pwd)) 
if 'Processed' in os.listdir(pwd):
    N_i-=1
columns = 3
rows = ((N_i-1)//columns)+1

I=np.linspace(1,max(im_i),len(im_i))
r56=(0.2e-3 *I**2)+ (46e-3* I) +2.59 #e-3 Current to R56 relation 


m=3.66466753e-02 #slope of pixel to wavelength relation
wl_0= 400 #central wavelength in nm
im = Image.open('28.395/andor_X300.tif')
pix=np.array(im)[100]
